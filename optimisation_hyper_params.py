...
def main():
    ...
    # Print final results
    print("Converged hyperparameters: alpha= %.6f, gamma= %.6f" %
...
def generate_data(xmin, xmax, Delta, noise):
    ...
    # generate x1 values from xmin to xmax
    x1 = np.arange(xmin, xmax+Delta, Delta)
    # generate x2 values from xmin to xmax
...
def KRR_function(hyperparams, X, y):
    ...
    print('alpha: %.6f . gamma: %.6f . rmse: %.6f' % (alpha_value,
                                                      gamma_value, rmse))  # Uncomment to print intermediate results
    return rmse
